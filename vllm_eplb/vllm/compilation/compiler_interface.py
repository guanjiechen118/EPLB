...
class InductorStandaloneAdaptor(CompilerInterface):
    ...
    def compile(
        ...
        if input_fake_mode is not None:
            # In PyTorch 2.9, ``torch._inductor.standalone_compile`` exposed
            # at package scope is a function, while the real implementation is
            # in the ``torch._inductor.standalone_compile`` module. Patch the
            # module object directly so both 2.9 and older layouts work.
            standalone_compile_mod = importlib.import_module(
                "torch._inductor.standalone_compile"
            )
            fake_mode_ctx: Any = patch.object(
                standalone_compile_mod,
                "FakeTensorMode",
