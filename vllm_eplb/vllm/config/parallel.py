...
class EPLBConfig:
    ...
    Supported values:
    - "swm": Sliding Window Mean (default). Equal-weight sum over a
    ...
    """Decay factor for EMA-based EPLB load estimation.

    Only used when ``algorithm="ema"``. Higher values give more weight to
    recent observations. Must be in ``(0, 1)``.
    """
    ...
    def _validate_parallel_config(self) -> Self:
        ...
        if self.enable_eplb:
            ...
            if self.eplb_config.algorithm == "ema" and not (
                ...
                raise ValueError(
                    "eplb_config.ema_alpha must be in (0, 1), but got "
                    f"{self.eplb_config.ema_alpha}."
                )
        else:
